...
export default function MiEquipo() {
    ...
    const handleGuardarPlantilla = async () => {
        ...
        try {
            ...
            console.log("Enviando plantilla para guardar:", {
                ...
                alineacionSeleccionada: selectedFormationLabel,
    ...
    const renderPlantillaSlots = useMemo(() => {
        ...
        currentFormation.order.forEach(posicionFrontend => {
            ...
            for (let i = 0; i < count; i++) {
                const currentPlayer = plantillaActual.get(globalIndex);
                slots.push(
                    <div key={globalIndex} className="relative w-28 h-40 border border-gray-600 rounded-lg flex flex-col items-center justify-center m-1 p-1 bg-gray-700 text-white shadow-md">
                        {currentPlayer ? (
                            <PlayerCard
                                carta={currentPlayer}
                                onClick={() => handleOpenPlayerSelectionModal(posicionFrontend, globalIndex)}
                                onEquipObject={() => {
                                    handleOpenObjectSelectionModal(currentPlayer, globalIndex);
                                }}
                            />
                        ) : (
    ...
    if (!manager) {
        return <div className="text-center text-yellow-500 text-xl mt-8">Por favor, inicia sesión para ver tu equipo.</div>;
    }
