...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("SELECT * FROM mydb.partido WHERE idPartido = ?", [params.id]) as [Partido[], any];
        if (!result.length) {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE mydb.partido SET idJornada = ?, idEquipoLocal = ?, idEquipoVisitante = ? WHERE idPartido = ?", [idJornada, idEquipoLocal, idEquipoVisitante, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM mydb.partido WHERE idPartido = ?", [params.id]) as [any, any];
