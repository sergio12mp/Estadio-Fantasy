...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE Partido SET idJornada = ?, idEquipoLocal = ?, idEquipoVisitante = ? WHERE idPartido = ?", [idJornada, idEquipoLocal, idEquipoVisitante, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM Partido WHERE idPartido = ?", [params.id]) as [any, any];
