...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE Manager_Ligas SET idLigas = ? WHERE idManager = ?", [idLigas, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM Manager_Ligas WHERE idManager = ?", [params.id]) as [any, any];
