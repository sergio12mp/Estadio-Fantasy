...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("SELECT * FROM Equipo WHERE idEquipo = ?", [params.id]) as [Equipo[], any];
        if (!result.length) {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE Equipo SET Nombre = ? WHERE idEquipo = ?", [Nombre, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM Equipo WHERE idEquipo = ?", [params.id]) as [any, any];
