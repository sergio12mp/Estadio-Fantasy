...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("SELECT * FROM Jugador WHERE idJugador = ?", [params.id]) as [Jugador[], any];
        if (!result.length) {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE Jugador SET Nombre = ?, Edad = ?, Pais = ?, Posicion = ?, Precio = ?, idEquipo = ? WHERE idJugador = ?", [Nombre, Edad, Pais, Posicion, Precio, idEquipo, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM Jugador WHERE idJugador = ?", [params.id]) as [any, any];
