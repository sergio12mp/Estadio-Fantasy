...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE Temporada SET Nombre = ? WHERE idTemporada = ?", [Nombre, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM Temporada WHERE idTemporada = ?", [params.id]) as [any, any];
