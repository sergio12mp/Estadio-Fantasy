...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("SELECT * FROM mydb.temporada WHERE idTemporada = ?", [params.id]) as [Temporada[], any];
        if (!result.length) {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const [result] = await db.query("UPDATE mydb.temporada SET Nombre = ? WHERE idTemporada = ?", [Nombre, params.id]) as [any, any];
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        const [result] = await db.query("DELETE FROM mydb.temporada WHERE idTemporada = ?", [params.id]) as [any, any];
